...
def test_parse_cmap():
    ...
    assert (common.parse_cmap(None, "reflectivity")
            in ("pyart_HomeyerRainbow", "pyart_ChaseSpectral"))
...
def test_generate_longitudinal_level_title():
    ...
    assert title == ("bar 0.0 km east of origin\n2000-01-01T00:00:00Z \nFoo")

    grid.x["data"][0] = -1000.0
    title = common.generate_longitudinal_level_title(grid, "foo", 0)
    assert title == ("bar 1.0 km west of origin\n2000-01-01T00:00:00Z \nFoo")
...
def test_generate_latitudinal_level_title():
    ...
    assert title == ("bar 0.0 km north of origin\n2000-01-01T00:00:00Z \nFoo")

    grid.y["data"][0] = -1000.0
    title = common.generate_latitudinal_level_title(grid, "foo", 0)
    assert title == ("bar 1.0 km south of origin\n2000-01-01T00:00:00Z \nFoo")
