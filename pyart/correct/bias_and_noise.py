...
def correct_visibility(radar, vis_field=None, field_name=None):
    ...
    corr_field_data = 10.0 * np.ma.log10(
        np.ma.power(10.0, 0.1 * field_data) * 100.0 / vis_data
    )
...
def get_sun_hits(
    ...
    if attg is None:
        ...
        if frequency_avail:
            attg = get_coeff_attg(radar.instrument_parameters["frequency"]["data"][0])
        else:
            attg = 0.0
    ...
    if ind_rmin.size > 0:
        ...
        warn(
            "Maximum radar range below the minimum range for sun signal"
            + " estimation. The last "
            + str(2 * nbins_min)
            + " will be inspected"
        )
    ...
    for ray in range(radar.nrays):
        ...
        if _PYSOLAR_AVAILABLE and sun_position == "pysolar":
            elev_sun, azim_sun = sun_position_pysolar(
                time[ray], radar.latitude["data"][0], radar.longitude["data"][0]
            )
        else:
        ...
        dazim = np.ma.abs(
            ...
            * np.ma.cos(elev_sun * np.pi / 180.0)
        )
        if dazim > 360.0:
            dazim -= 360.0
    ...
    for i in range(nhits):
        for sweep in range(new_radar.nsweeps):
            ...
            if (ray_start <= sun_hits["ray"][i]) and (ray_end >= sun_hits["ray"][i]):
                sweeps.append(sweep)
                break
...
def get_sun_hits_psr(
    ...
    for ray in range(radar.nrays):
        ...
        sun_hits["sun_az"].append(azim_sun)
        sun_hits["dBm_sun_hit"].append(sunpwrh_dBm)
        sun_hits["std(dBm_sun_hit)"].append(0.0)
        sun_hits["dBmv_sun_hit"].append(sunpwrv_dBm)
        sun_hits["std(dBmv_sun_hit)"].append(0.0)
...
def get_sun_hits_ivic(
    ...
    if radar.instrument_parameters is not None:
        if "number_of_pulses" in radar.instrument_parameters:
            ...
            warn(
                "Unknown number of pulses per ray. Default value "
                + str(npulses_ray)
                + " will be used for all rays"
            )
...
def sun_retrieval(
    ...
    if not is_zdr:
        ...
        if (
            (az_width_co is None)
            or (el_width_co is None)
            or (az_width_cross is None)
            or (el_width_cross is None)
        ):
            npar = 5
    ...
    par_aux, alpha, beta = gauss_fit(az_rad, az_sun, el_rad, el_sun, sun_hit, npar)
    ...
    if npar == 3:
        ...
        if is_zdr:
            par[3] = coeff * (
                1.0 / np.ma.power(az_width_cross, 2.0)
                - 1.0 / np.ma.power(az_width_co, 2.0)
            )
            par[4] = coeff * (
                1.0 / np.ma.power(el_width_cross, 2.0)
                - 1.0 / np.ma.power(el_width_co, 2.0)
            )
        else:
            par[3] = -np.ma.power(az_width_co, 2.0) / coeff
            par[4] = -np.ma.power(el_width_co, 2.0) / coeff
    else:
        par = par_aux

    if (not is_zdr) and (par[3] > 0.0 or par[4] > 0.0):
        warn(
            "Parameter ax and/or ay of Psun equation positive. "
...
def est_rhohv_rain(
    ...
    mask_refl = np.logical_or(
        np.ma.getmaskarray(refl), np.logical_or(refl < zmin, refl > zmax)
    )
...
def est_zdr_snow(
    ...
    if tempmin is not None and tempmax is not None and temp_field in radar.fields:
...
def selfconsistency_bias(
    ...
    if check_wet_radome:
        ...
        if refl_avg > wet_radome_refl and ngates_wet > wet_radome_ngates_min:
            warn(
                ...
                + str(radar.range["data"][wet_radome_len_min])
                + " and "
                + str(radar.range["data"][wet_radome_len_max])
                + " km "
                + str(refl_avg)
                + ". Number of wet gates "
                + str(ngates_wet)
    ...
    if keep_points:
        kdp = kdp_leastsquare_single_window(
            radar,
            wind_len=kdp_wind_len,
            min_valid=int(kdp_wind_len / 2.0),
            phidp_field=phidp_field,
            kdp_field=None,
            vectorize=True,
        )
        sm_refl = smooth_masked(
            refl, wind_len=smooth_wind_len, min_valid=1, wind_type="mean"
        )
        sm_refl_lin = np.ma.power(10.0, 0.1 * sm_refl)

        sm_zdr = smooth_masked(
            zdr, wind_len=smooth_wind_len, min_valid=1, wind_type="mean"
        )
        zdr_list = []
        zh_list = []
        kdp_list = []
    ...
    for ray in range(radar.nrays):
        ...
        # bias
        dphidp_obs = phidp[ray, ind_prec_cell[-1]] - phidp[ray, ind_prec_cell[0]]
        ...
        for i in range(len(ind_prec_cell)):
            ...
            dphidp_sim = (
                phidp_sim[ray, ind_prec_cell[-1 - i]] - phidp_sim[ray, ind_prec_cell[0]]
            )
...
def selfconsistency_bias2(
    ...
    for ray in range(radar.nrays):
        ...
        for ind_prec_cell in cons_list:
            ...
            kdp_sim.extend(kdp_sim_aux[ray, ind_prec_cell[0] : ind_prec_cell[-1] + 1])
            kdp_obs.extend(kdp[ray, ind_prec_cell[0] : ind_prec_cell[-1] + 1])

            if keep_points:
                zdr_list.extend(sm_zdr[ray, ind_prec_cell[0] : ind_prec_cell[-1] + 1])
                kdp_list.extend(kdp[ray, ind_prec_cell[0] : ind_prec_cell[-1] + 1])
                zh_list.extend(
...
def get_kdp_selfcons(zdr, refl, ele_vec, zdr_kdpzh_dict, parametrization="None"):
    """
    Estimates KDP and PhiDP in rain from  Zh and ZDR using a selfconsistency
    ...
    refl_lin = np.ma.power(10.0, refl / 10.0)
    ...
    if parametrization == "None":
        ...
        for i in range(len(zdr_kdpzh_dict["elev"])):
            ...
            kdpzh[ind_ray, :] = np.reshape(kdpzh_aux, (len(ind_ray), np.shape(zdr)[1]))
    ...
    if parametrization == "Vaccarono":
        if zdr_kdpzh_dict["freq_band"] == "C":
            ...
            return np.ma.power(np.ma.power(refl_lin, 0.91) * kdp085zh091, 1.0 / 0.85)
...
def _est_sun_hit_pwr_hs(pwr, sun_hit, attg_sun, max_std, nbins_min, ind_rmin):
    ...
    pwr_toa_mw = np.ma.power(10.0, 0.1 * (pwr[ind_rmin:] + attg_sun))
    ...
    sunpwr_std = np.ma.std(10.0 * np.ma.log10(pwr_valid))

    if sunpwr_std > max_std:
        warn(
            "Sun hit power not valid. Standard deviation "
            + str(sunpwr_std)
            + " larger than maximum expected"
        )
        return get_fillvalue(), get_fillvalue(), 0, nvalid, sun_hit

    sunpwr_dBm = 10.0 * np.ma.log10(sunpwr)
...
def _est_sun_hit_pwr_psr(pwr, attg_sun):
    ...
    pwr_toa_mw = np.ma.power(10.0, 0.1 * (pwr + attg_sun))
    sunpwr_dBm = 10.0 * np.ma.log10(pwr_toa_mw)

    return sunpwr_dBm


def _est_sun_hit_pwr_ivic(
    pwr,
    sun_hit,
    attg_sun,
    pct,
    ...
    pwr_toa_w = 1e-3 * np.ma.power(10.0, 0.1 * (pwr + attg_sun))
    sunpwr, _, _, inds_noise = estimate_noise_ivic13(
        pwr_toa_w,
        pct=pct,
        delay=2,
        flat_reg_wlen=flat_reg_wlen,
        flat_reg_var_max=flat_reg_var_max,
        snr_thr=snr_thr,
        npulses=npulses,
    ...
    sunpwr_dBm = 10.0 * np.ma.log10(sunpwr) + 30.0
...
def _est_sun_hit_zdr(
    zdr, sun_hit_zdr, sun_hit_h, sun_hit_v, max_std, nbins_min, ind_rmin
):
    """
    ...
    if sunzdr_std > max_std:
        warn(
            "Sun hit ZDR not valid. Standard deviation "
            + str(sunzdr_std)
            + " larger than maximum expected"
        )
        return get_fillvalue(), get_fillvalue(), 0, nvalid, sun_hit_zdr
...
def _selfconsistency_kdp_phidp(
    ...
    if min_rhohv is not None:
        mask_rhohv = np.logical_or(rhohv < min_rhohv, np.ma.getmaskarray(rhohv))
        mask = np.logical_or(mask, mask_rhohv)
