...
def correct_vpr(
    ...
    ml_thickness_max=800.0,
    ml_thickness_step=200.0,
    iso0_max=5000.0,
    ml_top_diff_max=200.0,
    ml_top_step=200.0,
    ml_peak_min=1.0,
    ml_peak_max=6.0,
    ml_peak_step=1.0,
    ...
    if iso0 is None:
        ...
        radar_azi_avg = compute_avg(
            ...
            temp_ref_field=temp_ref_field,
            lin_refl_field=lin_refl_field,
        )
    ...
    if filter_params:
        ...
        vpr_theo_dict = {
            "ml_top": best_ml_top,
            "ml_bottom": max(best_ml_top - best_ml_thickness, 0.0),
            "val_ml_peak": best_val_ml,
            "val_dr": best_val_dr,
        }

        vpr_theo_dict_filtered = filter_vpr_params(
            vpr_theo_dict, vpr_theo_dict_mem=vpr_theo_dict_mem, weight_mem=weight_mem
        )

        # get theoretical profile as a function of altitude
        vpr_theo_dict_filtered = compute_theoretical_vpr(
            ml_top=vpr_theo_dict_filtered["ml_top"],
            ml_thickness=vpr_theo_dict_filtered["ml_top"]
            - vpr_theo_dict_filtered["ml_bottom"],
            val_ml=vpr_theo_dict_filtered["val_ml_peak"],
            val_dr=vpr_theo_dict_filtered["val_dr"],
            h_max=h_corr_max,
            h_res=h_res,
        )
    else:
        ...
        vpr_theo_dict = compute_theoretical_vpr(
            ml_top=best_ml_top,
            ml_thickness=best_ml_thickness,
            val_ml=best_val_ml,
            val_dr=best_val_dr,
            h_max=h_corr_max,
            h_res=h_res,
    ...
    refl_corr_dict, corr_field_dict = compute_vpr_correction(
        radar,
        vpr_theo_dict_filtered,
        max_weight=max_weight,
        corr_field=corr_field,
        norm_refl_field=norm_refl_field,
...
def compute_theoretical_vpr(
    ml_top=3000.0, ml_thickness=200.0, val_ml=3.0, val_dr=-3.0, h_max=6000.0, h_res=1.0
):
    """
    ...
    val_theo[h < ml_bottom] = 1.0
    if ml_thickness > 0:
        val_theo[(h >= ml_bottom) & (h < ml_peak)] = 1.0 + 2.0 * (
            val_ml - 1.0
        ...
        val_theo[h > ml_top] = np.exp(
            np.log(10.0) / 10000.0 * val_dr * (h[(h > ml_top)] - ml_top)
        )
    else:
        val_theo[h >= ml_top] = np.exp(
            np.log(10.0) / 10000.0 * val_dr * (h[(h >= ml_top)] - ml_top)
...
def compute_apparent_vpr(
    ...
    half_bw_rad = half_bw * np.pi / 180.0
    weight_factor = -2.0 * np.log(2.0)
    ...
    for ind_ray, ang in enumerate(radar_out.elevation["data"]):
        if known_ml_btm:
            ...
            radar_out.fields[refl_field]["data"][ind_ray, 0:i_rng_btm] = 1.0
        else:
            i_rng_btm = 0

        rng_ml_vals = rng[i_rng_btm:] / 1000.0  # km

        # altitudes affected by the antenna diagram at each range gate
        _, _, z_diag_min_vals = antenna_to_cartesian(rng_ml_vals, 0.0, ang - half_bw)
        z_diag_min_vals += radar_out.altitude["data"]

        _, _, z_diag_max_vals = antenna_to_cartesian(rng_ml_vals, 0.0, ang + half_bw)
        z_diag_max_vals += radar_out.altitude["data"]

        _, _, z_diag_center_vals = antenna_to_cartesian(rng_ml_vals, 0.0, ang + half_bw)
...
def compute_avg(
    ...
    radar_aux = radar_aux.extract_sweeps(np.argsort(radar_aux.fixed_angle["data"]))

    # compute linear reflectivity
    refl_lin = get_metadata(lin_refl_field)
    refl_lin["data"] = np.ma.power(10.0, 0.1 * radar_aux.fields[refl_field]["data"])
    ...
    radar_rhi.fields[lin_refl_field]["data"][
        radar_rhi.gate_altitude["data"] >= h_max
    ] = np.ma.masked

    return radar_rhi


def compute_refl_time_avg(radar, refl_field=None, radar_mem_list=None, nvalid_min=20):
    """
    ...
    fields = {refl_field: radar_out.fields[refl_field]}
    ...
    return Radar(
        ...
        radar_out.longitude,
        radar_out.altitude,
        radar_out.sweep_number,
        radar_out.sweep_mode,
        radar_out.fixed_angle,
        radar_out.sweep_start_ray_index,
        radar_out.sweep_end_ray_index,
        radar_out.azimuth,
        radar_out.elevation,
    ...
    for ind_btm, ele_btm in enumerate(radar.elevation["data"][:-1]):
        for ele_top in radar.elevation["data"][ind_btm + 1 :]:
            ...
            ratio_aux = np.expand_dims(
                ...
                / radar.fields[refl_field]["data"][ind_btm, :],
                axis=0,
            )
            if first_ratio:
...
def find_best_profile(
    ...
    if iso0 >= iso0_max or iso0 <= radar_obs.altitude["data"]:
        warn(
            f"iso0 {iso0} masl higher than {iso0_max} m"
            f" or lower than the radar altitude"
        )
        return (best_vpr_theo_dict, iso0, 0.0, 1.0, dr_default, best_error)
    ...
    ml_peak_vals = np.arange(ml_peak_min, ml_peak_max + ml_peak_step, ml_peak_step)
    if iso0 + dr_alt >= h_max:
        warn(
            "theoretical profile not defined high enough above the melting"
    ...
    for val_ml in ml_peak_vals:
        if isclose(val_ml, 1.0):
            # special case where there is no melting layer peak
            ml_thickness_vals_aux = np.array([0.0])
        else:
...
def compute_vpr_correction(
    ...
    radar_corr_data = 10.0 * np.ma.log10(
        1.0 / radar_corr.fields[norm_refl_field]["data"]
    )

    corr_field_dict = get_metadata(corr_field)
    corr_field_dict["data"] = np.ma.zeros((radar.nrays, radar.ngates))
    for ind_ray, (ind_start, ind_end) in enumerate(
        zip(radar.sweep_start_ray_index["data"], radar.sweep_end_ray_index["data"])
    ):
        corr_field_dict["data"][ind_start : ind_end + 1, :] = radar_corr_data[
            ind_ray, :
        ]

    refl_corr_dict = get_metadata(corr_refl_field)
    refl_corr_dict["data"] = radar.fields[refl_field]["data"] + corr_field_dict["data"]

    return refl_corr_dict, corr_field_dict


def compute_vpr_correction_spatialised(
    radar,
    iso0_dict,
    vpr_theo_dict,
    max_weight=9.0,
    ...
    beam_width = radar.instrument_parameters["radar_beam_width_h"]["data"][0]
    half_bw = beam_width / 2.0
    half_bw_rad = half_bw * np.pi / 180.0
    weight_factor = -2.0 * np.log(2.0)

    rng_ml_vals = radar.range["data"] / 1000.0  # km
    for ind_sweep, ang in enumerate(radar.fixed_angle["data"]):
        ...
        for i_rng, (rng_val, z_diag_min, z_diag_max, z_diag_center) in enumerate(
            ...
            corr_data = compute_vpr_corr_rng_loop(
                ...
                vpr_theo_dict,
                ind_h_offset[ind_start : ind_end + 1, i_rng],
                rng_val,
                z_diag_min,
                z_diag_center,
                z_diag_max,
...
def compute_vpr_corr_rng(
    ...
    print(
        f"range: {rng_val} VPR theo samples: {ind_h[-1]}"
        f" min alt: {z_diag_min} max alt: {z_diag_max}",
        end="\r",
        flush=True,
    )

    weights = (vpr_theo_dict["altitude"][ind_h] - z_diag_center) / (
        rng_val * half_bw_rad
    )
    weights = weights * weights
    weights = np.ma.masked_greater(weights, max_weight)
    ...
    return 10.0 * np.ma.log10(weight_sum / np.ma.sum(vpr_vals * weights, axis=-1))


def compute_vpr_corr_rng_loop(
    refl_sweep,
    vpr_theo_dict,
    ind_h_offset_sweep,
    rng_val,
    ...
    return 10.0 * np.ma.log10(corr_data)
...
def filter_vpr(vpr_theo_dict, vpr_theo_dict_mem=None, weight_mem=0.75):
    ...
    if np.array_equal(vpr_theo_dict["altitude"], vpr_theo_dict_mem["altitude"]):
        vals = (1.0 - weight_mem) * vpr_theo_dict[
            "value"
...
def filter_vpr_params(vpr_theo_dict, vpr_theo_dict_mem=None, weight_mem=0.75):
    ...
    vpr_filt_dict["ml_top"] = (1.0 - weight_mem) * vpr_theo_dict[
        "ml_top"
    ] + weight_mem * vpr_theo_dict_mem["ml_top"]
    vpr_filt_dict["ml_bottom"] = (1.0 - weight_mem) * vpr_theo_dict[
        "ml_bottom"
    ] + weight_mem * vpr_theo_dict_mem["ml_bottom"]
    vpr_filt_dict["val_ml_peak"] = (1.0 - weight_mem) * vpr_theo_dict[
        "val_ml_peak"
    ] + weight_mem * vpr_theo_dict_mem["val_ml_peak"]
    vpr_filt_dict["val_dr"] = (1.0 - weight_mem) * vpr_theo_dict[
        "val_dr"
    ] + weight_mem * vpr_theo_dict_mem["val_dr"]
    return vpr_filt_dict


def correct_iso0_field(
    radar,
    ...
    if temp_ref == "height_over_iso0":
        ...
        temp_error = iso0_error * lapse_rate / 1000.0
