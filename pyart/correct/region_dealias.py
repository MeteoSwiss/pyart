...
def _find_sweep_interval_splits(nyquist, interval_splits, velocities, nsweep):
    ...
    if len(velocities) != 0:
        ...
        if max_vel > nyquist or min_vel < -nyquist:
            msg = f"Velocities outside of the Nyquist interval found in sweep {int(nsweep)}."
...
def _cost_function(
    ...
    for reg in range(nfeatures):
        ...
        add_value2 = np.square(
            vels_slice_means[reg] + np.round(nyq_vector[i]) * v_nyq_vel - vel_wo_cur
        )
        if np.isfinite(add_value2):
            add_value += 0.1 * add_value2
...
def _gradient(nyq_vector, vels_slice_means, svels_slice_means, v_nyq_vel, nfeatures):
    ...
    for reg in range(nfeatures):
        ...
        if np.isfinite(add_value2):
            gradient_vector[i] += 2 * 0.1 * add_value2 * v_nyq_vel
