...
def read_cfradial2(
    ...
    **kwargs
    ...
    filemetadata = FileMetadata(
        "cfradial2", field_names, additional_metadata, file_field_names, exclude_fields
    )
    ...
    sweep_start_ray_index["data"] = np.array(
        [_ncvar_to_dict(sweep_vars["ray_index"])["data"][0]], dtype="int"
    )
    sweep_end_ray_index["data"] = np.array(
        [_ncvar_to_dict(sweep_vars["ray_index"])["data"][-1]], dtype="int"
    )
    ...
    if "ray_n_gates" in sweep_vars:
        ...
        keys = [k for k, v in sweep_vars.items() if v.dimensions == ("n_points",)]
    else:
        # all variables with dimensions of 'time', 'range' are fields
        keys = [k for k, v in sweep_vars.items() if v.dimensions == ("time", "range")]
    ...
    for i, sweep_name in enumerate(sweep_group_name["data"][1:]):
        ...
        sweep_mode["data"] = np.append(
            sweep_mode["data"], _ncvar_to_dict(sweep_vars["sweep_mode"])["data"]
        )
        ...
        azimuth["data"] = np.append(
            azimuth["data"], _ncvar_to_dict(sweep_vars["azimuth"])["data"]
        )
        elevation["data"] = np.append(
            elevation["data"], _ncvar_to_dict(sweep_vars["elevation"])["data"]
        )
        if "scan_rate" in sweep_vars:
            scan_rate["data"] = np.append(
                scan_rate["data"], _ncvar_to_dict(sweep_vars["scan_rate"])["data"]
            )
        else:
            scan_rate = None
        ...
        if "rotation" in sweep_vars:
            rotation["data"] = np.append(
                rotation["data"], _ncvar_to_dict(sweep_vars["rotation"])["data"]
            )
        else:
            rotation = None
        ...
        if "tilt" in sweep_vars:
            tilt["data"] = np.append(
                tilt["data"], _ncvar_to_dict(sweep_vars["tilt"])["data"]
            )
        else:
            tilt = None
        ...
        if "roll" in sweep_vars:
            roll["data"] = np.append(
                roll["data"], _ncvar_to_dict(sweep_vars["roll"])["data"]
            )
        else:
            roll = None
        ...
        if "drift" in sweep_vars:
            drift["data"] = np.append(
                drift["data"], _ncvar_to_dict(sweep_vars["drift"])["data"]
            )
        else:
            drift = None
        ...
        if "heading" in sweep_vars:
            heading["data"] = np.append(
                heading["data"], _ncvar_to_dict(sweep_vars["heading"])["data"]
            )
        else:
            heading = None
        ...
        if "pitch" in sweep_vars:
            pitch["data"] = np.append(
                pitch["data"], _ncvar_to_dict(sweep_vars["pitch"])["data"]
            )
        else:
            pitch = None
        ...
        if "ray_n_gates" in sweep_vars:
            ...
            # all variables with dimensions of 'time', 'range' are fields
            keys = [
                k for k, v in sweep_vars.items() if v.dimensions == ("time", "range")
            ]
        ...
        for key in keys:
            ...
            if not delay_field_loading:
                fields[field_name]["data"] = np.ma.append(
                    ...
                    _ncvar_to_dict(sweep_vars[key], delay_field_loading)["data"],
                    axis=0,
                )
