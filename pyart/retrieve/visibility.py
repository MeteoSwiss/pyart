...
def _worker_function(
    angles, ranges, dem_data, radar_params, numeric_params, atm_params
):
    ...
    for ph, pv in quad_pts_GH:  # Loop on quadrature points
        ...
        xr, yr, zr = rad_to_cart(ranges, angles[0] + ph, angles[1] + pv, ke=ke)
        ...
        ray.calc_visibility()
        if ray.topography_encounter_idx is not None:
        ...
        # aggregate
        visib = np.nanmean(
            np.reshape(ray.visibility, (original_range_length, quad_pts_range)), axis=1
        )
        rcs = np.nanmean(
            np.reshape(ray.rcs, (original_range_length, quad_pts_range)), axis=1
        )
        clutter_W = np.nanmean(
            np.reshape(ray.clutter_W, (original_range_length, quad_pts_range)), axis=1
        )
        clutter_Z = np.nanmean(
            np.reshape(ray.clutter_Z, (original_range_length, quad_pts_range)), axis=1
        )
    ...
    # Convert mm6m-3 -> dBZ
    clutter_dBZ_integ = 10 * np.log10(clutter_dBZ_integ)
...
class _IndexNNGrid:
    ...
    def __init__(self, x, y):
        ...
        II = np.arange(len(x))
        JJ = np.arange(len(y))
        self.interp_i = interp1d(x, II, bounds_error=False, kind="nearest")
        self.interp_j = interp1d(y, JJ, bounds_error=False, kind="nearest")
...
class _Ray:
    ...
    def calc_dem_bent(self, dem_data, alt):
        ...
        self.dem[self.valid_idx] = dem_data[self.i_idx_valid, self.j_idx_valid]
    ...
    def calc_slope_and_aspect(self, dem_data, grid_res, onlytopo=False):
        ...
        slope[valid] = np.arctan(np.sqrt(gy[valid] ** 2 + gx[valid] ** 2)) * 180 / np.pi
        aspect = np.ma.zeros(self.N) + np.nan
        aspect[valid] = (np.arctan2(gy[valid], -gx[valid]) + np.pi) * 180 / np.pi
    ...
    def calc_theta(self, onlytopo=False):
        ...
        theta[valid] = (
            np.arctan2(self.dem_bent[valid], self.ranges[valid]) * 180 / np.pi
        )
        self.theta = theta
    ...
    def calc_min_theta(self, beamwidth):
        ...
        min_vis_theta[i] = current + beamwidth / 2.0
    ...
    def calc_min_vis_alt(self, ke):
        ...
        self.min_vis_alt = (
            self.ranges**2 + R**2 + 2.0 * self.ranges * R * np.sin(theta_e)
        ) ** 0.5 - R
    ...
    def calc_incident_ang(self, onlytopo=False):
        ...
        zenith = (90.0 - self.theta[valid]) * np.pi / 180.0
    ...
    def calc_eff_area(self, grid_res, onlytopo=False):
        ...
        area_eff[valid] = grid_res**2 / np.cos(self.slope[valid] * np.pi / 180.0)
    ...
    def calc_clutter_Z(self, freq, mosotti, tau, beamwidth):
        ...
        self.clutter_Z = (
            self.rcs - 20 * self.ranges_log + dbzconst + convert_dbzm_to_dbz
        )

    def calc_sigma_0(self, method="gabella", freq_ghz=5.6, onlytopo=False):
        """
        ...
        if method == "gabella":
            ...
            factor = (
                k1 * np.cos(te2r) * ((np.pi / 2.0 - te2r) / (np.pi / 2.0 - te1r)) ** k2
            )
            ...
            if len(ind1):
                ...
                sigma_0[ind1] = (
                    k1
                    * np.cos(iang)
                    * ((np.pi / 2.0 - iang) / (np.pi / 2.0 - te1r)) ** k2
                )
            ...
            if len(ind):
                iang = inc_angr[ind]
                sigma_db[ind] = a1 + b1 * iang + c1 * freq_ghz + d1 * freq_ghz * iang
            ind = np.where(inc_ang >= lim_ang_del)[0]
            if len(ind):
                iang = inc_angr[ind]
                sigma_db[ind] = (
                    a1_2 + b1_2 * iang + c1_2 * freq_ghz + d1_2 * freq_ghz * iang
                )

            sigma_0 = 10 ** (sigma_db / 10.0)
        else:
            warnings.warn('Invalid method for sigma_0: use "gabella" or ' + '"delrieu"')
            return
...
def visibility_processing(
    ...
    for i, fixangle in enumerate(radar.fixed_angle["data"]):
        ...
        visibility[idx : idx + len(results), :] = np.array([r[0] for r in results])
        clutter_rcs[idx : idx + len(results), :] = np.array([r[1] for r in results])
        clutter_dBm[idx : idx + len(results), :] = np.array([r[2] for r in results])
        clutter_dBZ[idx : idx + len(results), :] = np.array([r[3] for r in results])
    ...
    return (
        ...
        incident_angle_dic,
        sigma_0_dic,
        rcs_ground_clutter_dic,
        dBm_ground_clutter_dic,
