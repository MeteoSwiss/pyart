...
def vad_michelson(radar, vel_field=None, z_want=None, gatefilter=None, sign=1):
    """
...
def vad_browning(
    ...
    """
    Velocity azimuth display.
    Note: This code is a multi-sweep (volume) version of the Browning VAD code
    It computes the VAD separately for every sweep and averages the results
...
def _vad_browning(
    ...
    """
    Velocity azimuth display.
    Note: This code uses only one sweep.
    To compute the VAD for multiple sweep use the vad_browning function
    ...
    vel_std_interp = _Average1D(
        good_height, good_vel_std, z_want[1] - z_want[0] / window, weight
    )

    u_wanted = u_interp(z_want)
    v_wanted = v_interp(z_want)
    vel_std_wanted = vel_std_interp(z_want)
    u_wanted = np.ma.masked_equal(u_wanted, 99999.0)
    v_wanted = np.ma.masked_equal(v_wanted, 99999.0)
    vel_std_wanted = np.ma.masked_equal(vel_std_wanted, 99999.0)
