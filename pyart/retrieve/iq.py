...
def compute_pol_variables_iq(radar, fields_list, subtract_noise=False, lag=0,
    ...
    if ('reflectivity' in fields_list or
            ...
            ('cross_correlation_ratio' in fields_list and lag == 0) or
            ('spectrum_width' in fields_list and lag == 0)):
        noise_h = None
        if noise_h_field in radar.fields:
            noise_h = radar.fields[noise_h_field]['data']
        pwr_h = _compute_power(
            radar.fields[signal_h_field]['data'], noise=noise_h,
            subtract_noise=subtract_noise)

    if ('reflectivity_vv' in fields_list or
            'differential_reflectivity' in fields_list or
            ('cross_correlation_ratio' in fields_list and lag == 0) or
            ('spectrum_width_vv' in fields_list and lag == 0)):
        noise_v = None
        if noise_v_field in radar.fields:
            noise_v = radar.fields[noise_v_field]['data']
        pwr_v = _compute_power(
            radar.fields[signal_v_field]['data'], noise=noise_v,
            subtract_noise=subtract_noise)

    if ('velocity' in fields_list or 'spectrum_width' in fields_list or
            ('differential_reflectivity' in fields_list and lag == 1) or
            ('cross_correlation_ratio' in fields_list and lag == 1)):
    ...
    if ('velocity_vv' in fields_list or 'spectrum_width_vv' in fields_list or
            ('differential_reflectivity' in fields_list and lag == 1) or
            ('cross_correlation_ratio' in fields_list and lag == 1)):
    ...
    if lag > 0 and ('differential_reflectivity' in fields_list or
                    'cross_correlation_ratio' in fields_list):
        rlagn_h_abs = np.ma.abs(
            _compute_autocorrelation(radar, signal_h_field, lag=1))
...
def compute_reflectivity_iq(radar, subtract_noise=False, signal_field=None,
                            noise_field=None):
    """
    ...
    if pol == 'hh':
        if ('dBADU_to_dBm_hh' not in radar.radar_calibration or
                'calibration_constant_hh' not in radar.radar_calibration):
            raise ValueError(
                'Unable to compute reflectivity. ' +
                'Calibration parameters unknown')
        dBADU2dBm = radar.radar_calibration['dBADU_to_dBm_hh']['data'][0]
        radconst = (
            radar.radar_calibration['calibration_constant_hh']['data'][0])
    else:
        if ('dBADU_to_dBm_vv' not in radar.radar_calibration or
                'calibration_constant_vv' not in radar.radar_calibration):
            raise ValueError(
                'Unable to compute reflectivity. ' +
                'Calibration parameters unknown')
    ...
    if (pol == 'hh' and 'matched_filter_loss_h' in radar.radar_calibration):
        mfloss = radar.radar_calibration['matched_filter_loss_h']['data'][0]
          ...
          'matched_filter_loss_v' in radar.radar_calibration):
...
def compute_differential_reflectivity_iq(radar, subtract_noise=False, lag=0,
    ...
    if ('dBADU_to_dBm_hh' not in radar.radar_calibration or
            'dBADU_to_dBm_vv' not in radar.radar_calibration or
            'calibration_constant_hh' not in radar.radar_calibration or
            'calibration_constant_vv' not in radar.radar_calibration):
        raise ValueError(
            'Unable to compute spectral reflectivity. ' +
            'Calibration parameters unknown')
    ...
    if lag == 0:
        ...
        pwr_h = _compute_power(
            radar.fields[signal_h_field]['data'], noise=noise,
            subtract_noise=subtract_noise)
        ...
        pwr_v = _compute_power(
            radar.fields[signal_v_field]['data'], noise=noise,
            subtract_noise=subtract_noise)
    else:
        pwr_h = np.ma.abs(
            _compute_autocorrelation(radar, signal_h_field, lag=lag))
        pwr_v = np.ma.abs(
...
def compute_rhohv_iq(radar, subtract_noise=False, lag=0, signal_h_field=None,
                     signal_v_field=None, noise_h_field=None,
                     noise_v_field=None):
    """
...
def compute_Doppler_velocity_iq(radar, signal_field=None,
    ...
    if ('prt' not in radar.instrument_parameters or
            'frequency' not in radar.instrument_parameters):
        warn('Unable to compute Doppler velocity. '
             'Unknown PRT or radar frequency')
        return None
...
def compute_Doppler_width_iq(radar, subtract_noise=True, signal_field=None,
                             noise_field=None, lag=1):
    """
...
def _compute_power(signal, noise=None, subtract_noise=False):
    ...
    """
    pwr = np.ma.mean(np.ma.power(np.ma.abs(signal), 2.), axis=-1)
