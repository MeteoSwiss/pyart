...
def _kdp_vulpiani_profile(psidp_in, dr, windsize=10, band="X", n_iter=10, interp=False):
    ...
    kdp_calc[l2 : nn - l2] = (psidp[ll:nn] - psidp[0 : nn - ll]) / (2.0 * ll * drm)
    ...
    for i in range(0, n_iter):
        ...
        # In the core of the profile
        kdp_calc[l2 : nn - l2] = (phidp_rec[ll:nn] - phidp_rec[0 : nn - ll]) / (
            2.0 * ll * drm
        )
...
def kdp_leastsquare_single_window(
    radar, wind_len=11, min_valid=6, phidp_field=None, kdp_field=None, vectorize=False
):
    """
...
def leastsquare_method_scan(phidp, rng_m, wind_len=11, min_valid=6):
    ...
    rng_mat = rolling_window(rng_mat / 1000.0, wind_len)
    ...
    rng_sum2 = np.ma.sum(np.ma.power(rng_wind_ma, 2.0), -1)[ind_valid]
    ...
    kdp[ind_valid[0], ind_valid[1] + half_wind] = (
        0.5
        * (rphidp_sum - rng_sum * phidp_sum / nvalid)
...
def leastsquare_method(phidp, rng_m, wind_len=11, min_valid=6):
    ...
    rng_wind = rolling_window(rng_m / 1000.0, wind_len)
