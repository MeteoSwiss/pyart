...
def colocated_gates(radar1, radar2, h_tol=0.0, latlon_tol=0.0, coloc_gates_field=None):
    ...
    x0, y0 = geographic_to_cartesian_aeqd(
        ...
        radar2.longitude["data"][0],
        radar2.latitude["data"][0],
        R=6370997.0,
    )
...
def colocated_gates2(
    radar1, radar2, distance_upper_bound=1000.0, coloc_gates_field=None
):
    """
    ...
    tree = cKDTree(
        np.transpose((rad2_x, rad2_y, rad2_z)), compact_nodes=False, balanced_tree=False
    )
    ...
    # not colocated: set co-located flag to 1
    coloc_rad1["data"][ind_ray_rad1[ind_not_col], ind_rng_rad1[ind_not_col]] = 1
...
def intersection(
    ...
    if vol_d_tol is not None:
        intersec_rad1[
            np.logical_not(find_equal_vol_region(radar1, radar2, vol_d_tol=vol_d_tol))
        ] = 1
    ...
    if azmin is not None and azmax is not None:
        ...
        if azmin > azmax:
            intersec_rad1[
                np.logical_and(
                    radar1.azimuth["data"] < azmin, radar1.azimuth["data"] > azmax
                ),
                :,
            ] = 1
...
def find_intersection_volume(radar1, radar2, h_tol=0.0, latlon_tol=0.0):
    """
    ...
    min_lat, max_lat, min_lon, max_lon, min_alt, max_alt = find_intersection_limits(
        radar1.gate_latitude["data"],
        radar1.gate_longitude["data"],
        radar1.gate_altitude["data"],
        radar2.gate_latitude["data"],
        radar2.gate_longitude["data"],
        radar2.gate_altitude["data"],
        h_tol=h_tol,
    ...
    intersec[
        np.logical_and(
            np.logical_and(
                radar1.gate_altitude["data"] > min_alt,
                radar1.gate_altitude["data"] < max_alt,
            ),
            np.logical_and(
                np.logical_and(
                    radar1.gate_latitude["data"] > min_lat,
                    radar1.gate_latitude["data"] < max_lat,
                ),
                np.logical_and(
                    radar1.gate_longitude["data"] > min_lon,
                    radar1.gate_longitude["data"] < max_lon,
                ),
            ),
        )
    ] = 2
...
def find_intersection_limits(
    lat1, lon1, alt1, lat2, lon2, alt2, h_tol=0.0, latlon_tol=0.0
):
    """
...
def find_equal_vol_region(radar1, radar2, vol_d_tol=0):
    ...
    if (
        radar2.instrument_parameters is not None
        and "radar_beam_width_h" in radar2.instrument_parameters
    ):
        ...
        bwidth2 = 1.0
    if (
        radar1.instrument_parameters is not None
        and "radar_beam_width_h" in radar1.instrument_parameters
    ):
    ...
    return np.isclose(vol_d_rad1, vol_d_rad2, rtol=0.0, atol=vol_d_tol)
...
def get_ground_distance(lat_array, lon_array, lat0, lon0):
    ...
    dlat_rad = (lat_array - lat0) * np.pi / 180.0
    dlon_rad = (lon_array - lon0) * np.pi / 180.0
    a = np.sin(dlat_rad / 2.0) * np.sin(dlat_rad / 2.0) + np.cos(
        lat_array * np.pi / 180.0
    ) * np.cos(lat0 * np.pi / 180.0) * np.sin(dlon_rad / 2.0) * np.sin(dlon_rad / 2.0)

    return 2.0 * np.arctan2(np.sqrt(a), np.sqrt(1.0 - a)) * r_earth
...
def get_vol_diameter(beamwidth, rng):
    ...
    return beamwidth * np.pi / 180.0 * rng
...
def get_target_elevations(radar):
    ...
    target_elevations = np.sort(radar.elevation["data"][sweep_start : sweep_end + 1])
...
def _construct_xsect_radar(
    ...
    scan_type,
    pxsect_rays,
    xsect_sweep_start_ray_index,
    xsect_sweep_end_ray_index,
    target_angles,
):
    """
...
def interpolate_pts_xsect(ref_points, bin, exact=True):
    ...
    for d in ipts_dist:
        ...
        pt_lon, pt_lat, _ = geod.fwd(
            ref_points[idx, 0], ref_points[idx, 1], az[idx], offset
        )
