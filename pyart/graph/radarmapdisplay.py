...
def _add_populated_places(ax, resolution="10m"):
    ...
    ind = np.where(
        np.logical_and(
            np.logical_and(lon_town > map_extent[0], lon_town < map_extent[1]),
            np.logical_and(lat_town > map_extent[2], lat_town < map_extent[3]),
        )
    )[0]
