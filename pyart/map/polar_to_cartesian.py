...
def polar_to_cartesian(
    radar_sweep, field_name, cart_res=75, max_range=None, mapping=None
):
    """
    ...
    if is_ppi:
        ...
        x_vec = np.arange(
            min([(max_range - cart_res) * np.cos(np.radians(np.max(theta))), 0]),
            max_range + cart_res,
            cart_res,
        )
    ...
    if is_ppi:
        ...
        r_grid_c = np.sqrt(
            (E * KE * np.sin(np.radians(theta_grid_c))) ** 2
            + 2 * E * KE * y_grid_c
            + y_grid_c**2
    ...
    if not mapping:
        ...
        _, mapping_idx = kdtree.query(
            np.vstack((r_grid_c.ravel(), theta_grid_c.ravel())).T, k=1
        )
