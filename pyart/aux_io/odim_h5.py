...
def read_odim_grid_h5(
    ...
    **kwargs,
    ...
    with h5py.File(filename, "r") as hfile:
        ...
        if (
            "LL_lat" in h_where
            and "LL_lon" in h_where
            and "UR_lat" in h_where
            and "UR_lon" in h_where
        ):
            ...
            xvec = (
                np.arange(0, h_where["xscale"] * h_where["xsize"], h_where["xscale"])
                + h_where["xscale"] / 2.0
                - h_where["xscale"] * h_where["xsize"] / 2.0
            )
            yvec = (
                np.arange(0, h_where["yscale"] * h_where["ysize"], h_where["yscale"])
                + h_where["xscale"] / 2.0
                - h_where["xscale"] * h_where["xsize"] / 2.0
            )
            ...
            for dset in datasets:
                for k in hfile[dset]:
                    if k.startswith("data"):
                        ...
                        odim_fields.append(hfile[dset][k]["what"].attrs["quantity"])
                        dsets.append(dset)
        ...
        for odim_field, h_field_key, dset in zip(odim_fields, h_field_keys, dsets):
            ...
            if fdata.ndim == 3:
                ...
                ny = h_where["ysize"]
                nx = h_where["xsize"]
                field_dic["data"] = ma_broadcast_to(fdata, (1, ny, nx))
    ...
    **kwargs,
    ...
    with h5py.File(filename, "r") as hfile:
        ...
        if "rstart" in hfile["dataset1/where"].attrs:
            ...
            _range["data"] = np.linspace(0, max_range[0] * 1000.0, max_nbins).astype(
                "float32"
            )
            _range["meters_to_center_of_first_gate"] = 0
            _range["meters_between_gates"] = max_range[0] * 1000.0 / max_nbins
        ...
        for dset, start, stop in zip(datasets, ssri, seri):
            if odim_object == "ELEV":
                ...
                sweep_az[sweep_az < 0.0] = 360 + sweep_az[sweep_az < 0.0]
            else:
...
def read_odim_vp_h5(
    ...
    **kwargs,
    ...
    with h5py.File(filename, "r") as hfile:
        ...
        # fixed_angle
        fixed_angle = filemetadata("fixed_angle")
        fixed_angle["data"] = np.array([90.0], dtype="float32")

        # elevation
        elevation = filemetadata("elevation")
        elevation["data"] = np.array([90.0], dtype="float32")
        ...
        _range["data"] = np.arange(rscenter, rscenter + rend, rscale, dtype="float32")
        ...
        # azimuth
        azimuth = filemetadata("azimuth")
        azimuth["data"] = np.array([0.0], dtype="float32")

        # time
        _time = filemetadata("time")
        _time["units"] = make_time_unit_str(
        ...
        for odim_field, h_field_key in zip(odim_fields, h_field_keys):
            ...
            for dset, rays_in_sweep in zip(datasets, rays_per_sweep):
                ...
                fdata[start : start + rays_in_sweep, :sweep_nbins] = sweep_data.T
                # set data to NaN if its beyond the range of this sweep
                fdata[start : start + rays_in_sweep, sweep_nbins:max_nbins] = np.nan
                start += rays_in_sweep
            # create field dictionary
...
def _get_odim_h5_sweep_data(
    group, offset=0, gain=1, nodata=np.nan, undetect=np.nan, use_file_conversion=True
):
    ...
    if use_file_conversion:
        try:
            ...
            warn(
                "Unable to use parameters to convert to physical units from"
