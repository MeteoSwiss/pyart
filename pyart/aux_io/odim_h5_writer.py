...
# obtained from OPERA Data Information Model for HDF5 (to be kept up-to-date)
CURRENT_IMAGE_VERSION = "1.2"
    ...
    for i in range(n_datasets):
        ...
        for j, field_name in enumerate(field_names):
            ...
            data_dict = _get_data_from_field(
                grid, i, field_name, physical=physical, undefined_value=undefined_value
            )
            ...
            # dictionary
            if (
                "labels" in grid.fields[field_name]
                and "ticks" in grid.fields[field_name]
            ):
...
def write_odim_h5(
    ...
    if not radar.ray_angle_res:
        ...
        for i in range(radar.nsweeps):
            ...
            azdiff = (
                radar.azimuth["data"][start + 1 : end]
                - radar.azimuth["data"][start : end - 1]
            ).mean()
            ...
            if len(set(radar.fixed_angle["data"])) <= 1:
                ...
                if radar.sweep_mode is not None:
                    if len(set(radar.sweep_mode["data"])) <= 1:
                        ...
                        warn(
                            "Radar sweep mode changes, which is not "
    ...
    if odim_object in ["PVOL", "AZIM", "SCAN", "ELEV"]:
        ...
        where2_dict["rstart"] = np.repeat(
            np.double(rstart / 1000.0), n_datasets
        )  # [km]
        ...
        if len(set(radar.range["data"][1:] - radar.range["data"][0:-1])) <= 1:
            ...
            warn(
                "Radar range resolution changes between gates. "
        ...
        # RHI specific
            ...
            where2_dict["range"] = np.repeat(
                np.double(
                    (np.max(radar.range["data"]) + (radar.range["data"][0])) / 1000.0
                ),
                n_datasets,
            )  # [km] to end of last gate
        ...
        if odim_object in ["PVOL", "AZIM", "SCAN"]:
            ...
            for i in range(n_datasets):
                ...
                T_start_az_tmp = (
                    radar.time["data"][ssri[i] : seri[i] + 1] - radar.time["data"][0]
                )
                T_stop_az_tmp = (
                    radar.time["data"][ssri[i] : seri[i] + 1] + radar.time["data"][0]
                )
            ...
            if radar.azimuth["data"] is not None:
                ...
                for i in range(n_datasets):
                    start_azA_tmp = (
                        ...
                        - radar.ray_angle_res["data"][i] / 2.0
                    )
                    start_azA_tmp[start_azA_tmp < 0.0] += 360.0  # [0 360]
                    stop_azA_tmp = (
                        radar.azimuth["data"][ssri[i] : seri[i] + 1]
                        + radar.ray_angle_res["data"][i] / 2.0
                    )
                    stop_azA_tmp[stop_azA_tmp > 360.0] -= 360.0  # [0 360]
            ...
            for idataset, how2_grp in enumerate(how2_grps):
                for name in how_var_dataset:
                    if how2_dict[name] is not None:
                        _create_odim_h5_attr(how2_grp, name, how2_dict[name][ind])
                if n_datasets == 1:
                    ind = ind + 1
                    continue
                if radar.instrument_parameters is not None:
                    ...
                    for name in how_var_instrument:
                        if name in how2_ins_dict:
                            _create_odim_h5_attr(how2_grp, name, how2_ins_dict[name])
                ind = ind + 1
        ...
        for i in range(n_datasets):
            ...
            for j, field_name in enumerate(field_names):
                ...
                # dictionary
                if (
                    "labels" in radar.fields[field_name]
                    and "ticks" in radar.fields[field_name]
                ):
...
def _get_sec_since_epoch(time_f):
    ...
    if hasattr(time_f, "microsecond"):
        sec_since_epoch = (
            calendar.timegm(time_f.timetuple()) + time_f.microsecond / 1000000.0
        )
    else:
...
def _create_odim_h5_dataset(
    ID, name, data_arr, make_legend=False, compression="gzip", compression_opts=6
):
    """
    ...
    """
    if make_legend:
        dt_tmp = [("label", h5py.special_dtype(vlen=str)), ("tick", np.float64)]
...
def _map_radar_quantity(field_name):
    ...
    odim_quantity_dict = {
        ...
        "corrected_path_integrated_differential_attenuation": "PIDAC",  # Non standard
    ...
    if field_name not in odim_quantity_dict:
        warn(
            "Field name "
            + field_name
            + " does not have an ODIM quantity "
...
def _get_data_from_field(
    radar, sweep_ind, field_name, physical=True, undefined_value=None
):
    """
    ...
    if physical:
        ...
        if ("scale_factor" in radar.fields[field_name]) and (
            ...
            if ind[0].size > 0:
                warn(
                    str(ind[0].size)
                    + " data points below the minimum value "
                    + str(val_min)
                )
                data_ph[ind] = val_min
            ...
            if ind[0].size > 0:
                warn(
                    str(ind[0].size)
                    + " data points above the maximum value "
                    + str(val_max)
                )
                data_ph[ind] = val_max
        else:
            ...
            warn(
                "gain and offset not specified. Optimal scale with gain "
                + str(gain)
                + " and offset "
                + str(offset)
                + " will be used"
...
def _map_radar_to_how_dict(radar_obj, ind_data=0):
    ...
    _RADAR_METADATA = ["system", "software", "sw_version"]
    ...
    for key in radar_obj.keys():
        if key in _INSTRUMENT_PARAMS:
            ...
            if key == "prt":
                dict_odim["prt"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "prt_ratio":
                dict_odim["prt_ratio"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "nyquist_velocity":
                dict_odim["NI"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "n_samples":
                dict_odim["Vsamples"] = np.int64(radar_obj[key]["data"][ind_data])
            if key == "radar_antenna_gain_h":
                dict_odim["antgainH"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "radar_antenna_gain_v":
                dict_odim["antgainV"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "calibration_constant_hh":
                dict_odim["radconstH"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "calibration_constant_vv":
                dict_odim["radconstV"] = np.double(radar_obj[key]["data"][ind_data])
            if key == "radar_beam_width_h":
                dict_odim["beamwH"] = np.double(radar_obj[key]["data"][0])
            if key == "radar_beam_width_v":
                dict_odim["beamwV"] = np.double(radar_obj[key]["data"][0])
            if key in ["radar_receiver_bandwidth", "radar_rx_bandwidth"]:
                dict_odim["RXbandwidth"] = np.double(radar_obj[key]["data"][ind_data])
