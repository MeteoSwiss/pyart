...
MF_FIELD_NAMES_DICT = {"data_quality": "QUALITY", "radar_estimated_rain_rate": "ACRR"}


def read_cf1_cartesian(
    filename,
    field_names=None,
    ...
    **kwargs,
    ...
    reserved_variables = [
        ...
        "nominal_wavelength0",
        "nominal_wavelength1",
        "nominal_wavelength10",
        "nominal_wavelength11",
        "nominal_wavelength12",
        "nominal_wavelength2",
        "nominal_wavelength3",
        "nominal_wavelength4",
        "nominal_wavelength5",
    ...
    x["data"] = (
        1000.0 * (np.arange(nx) + chy0 + 1 / 2.0) - grid_mapping["false_easting"]
    )

    y["data"] = (
        1000.0 * (np.arange(ny) + chx0 + 1 / 2.0) - grid_mapping["false_northing"]
    )

    z["data"] = np.array([0.0])

    # Origin of LV03 Swiss coordinates
    origin_latitude["data"] = np.array([grid_mapping["latitude_of_projection_origin"]])
    ...
    for field in field_names:
        ...
        if field_dic_file["data"].shape == field_shape:
            field_dic_file["data"] = np.expand_dims(
                np.squeeze(field_dic_file["data"], axis=-1), axis=0
            )[:, ::-1, :]
            ...
            warn(f"Field {field} skipped due to incorrect shape {bad_shape}")
...
def read_cf1_cartesian_mf(
    filename, field_names=None, delay_field_loading=False, **kwargs
):
    """
    Read a CF-1 netCDF file.
    ...
    reserved_variables = [
        ...
        "data_mf_products",
        "radar_image_time",
        "radar_image_latitude",
        "radar_image_longitude",
        "radar_image_wmoid",
        "radar_image_name",
    ]
