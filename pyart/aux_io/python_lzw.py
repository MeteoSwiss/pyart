...
class BitPacker():
    ...
    def pack(self, codepoints):
        ...
        >>> [ b for b in pkr.pack([ 1, 257]) ] == [ struct.Struct(">B").pack(0), struct.Struct(">B").pack(0xC0), struct.Struct(">B").pack(0x40) ]
        True
        """
        tailbits = []
...
class BitUnpacker():
    ...
    def unpack(self, bytesource):
        ...
        >>> [ i for i in unpk.unpack([ struct.Struct(">B").pack(0), struct.Struct(">B").pack(0xC0), struct.Struct(">B").pack(0x40) ]) ]
        [1, 257]
        """
        bits = []
        offset = 0
...
class PagingEncoder():
    """
    UNTESTED. Handles encoding of multiple chunks or streams of encodable
    data, separated with control codes. Dual of PagingDecoder.
    """
...
class PagingDecoder():
    """
    UNTESTED. Dual of PagingEncoder, knows how to handle independantly
    encoded, END_OF_INFO_CODE delimited chunks of an inbound byte stream
    """
