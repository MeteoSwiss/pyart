...
def read_hpl(filename, additional_metadata=None, location=None):
    ...
    with open(filename) as fo:
        for i, line in enumerate(fo):
            ...
            if endheader:
                ...
                if len(lsplit[0]) > 4:
                    _time["data"].append(
                        day + datetime.timedelta(hours=float(lsplit[0]))
                    )
    ...
    for k in fields.keys():
        fields[k]["data"] = np.reshape(
            np.array(fields[k]["data"]),
            (len(azimuth["data"]), metadata["Number of gates"]),
        )

    _range["data"] = metadata["Range gate length (m)"] * np.arange(
    ...
    sweep_end_ray_index["data"] = np.array([azimuth["data"].size - 1], dtype=np.int32)
    ...
    if nele_unique.size == 1 and nazi_unique.size == 1:
        ...
        raise ValueError("Only single sweeps PPI, RHI or pointing are supported")
